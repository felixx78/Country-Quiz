...
function App() {
  ...
  useEffect(() => {
    setTimeout(() => {
      getQuestions(10).then((questions) => setQuestions(questions));
    }, 1000);
  }, []);
  ...
  const selectAnswer = (answer: string, questionIndex: number) => {
    const newSelectedAnswerList = selectedAnswerList.slice();
    newSelectedAnswerList[questionIndex] = answer;
    setSelectedAnswerList(newSelectedAnswerList);
  };

  const resetQuiz = () => {
    setCorrectAnswersCount(0);
    setSelectedAnswerList(Array.from({ length: 10 }, () => ""));
  ...
  return (
    <div
      className="text-[#E2E4F3] font-semibold min-h-screen h-full"
      style={{
        background: `url(${bg})`,
        backgroundSize: "cover",
        backgroundRepeat: "no-repeat",
    ...
    >
      {showCongratsPage ? (
        ...
        <QuizPage
          ...
          onLastQuestion={() => setShowCongratsPage(true)}
        />
      )}
    </div>
  );
